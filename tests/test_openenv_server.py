...
class OpenEnvServerTests(unittest.TestCase):
    ...
    def test_http_metadata_endpoints_expose_tasks(self) -> None:
        ...
        self.assertIn("description", root_response.json())
        ...
        self.assertIn("description", info_response.json())
        ...
        self.assertEqual(metadata_response.json()["name"], "tron")
        self.assertIn("description", metadata_response.json())
        self.assertEqual([item["id"] for item in metadata_response.json()["tasks"]], ["easy", "medium", "hard"])
        self.assertTrue(all(item["score_range"] == [0.001, 0.999] for item in metadata_response.json()["tasks"]))
    ...
    def test_http_grade_returns_deterministic_fallback_when_cluster_is_unreachable(self) -> None:
